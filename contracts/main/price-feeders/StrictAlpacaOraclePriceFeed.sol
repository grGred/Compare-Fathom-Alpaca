...
contract StrictAlpacaOraclePriceFeed is PausableUpgradeable, AccessControlUpgradeable, IPriceFeed {
    ...
    function peekPrice() external view override returns (bytes32, bool) {
        (uint256 primaryPrice, uint256 primaryLastUpdate) = primary.alpacaOracle.getPrice(
            primary.token0,
            primary.token1
        );
        ...
        return (
            bytes32(primaryPrice),
            _isPriceOk(primaryPrice, secondaryPrice, primaryLastUpdate, secondaryLastUpdate)
        );
    }
